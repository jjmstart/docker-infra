...
| 备份恢复最小闭环                                      | v1.7  | ✅ 已完成   |
| Dead Man's Switch（告警出口冗余）                     | v1.8  | 待开始     |
| 基础可复现性修复（Docker 网络 + MySQL 数据层 bootstrap）       | v1.9  | 待开始     |
| ProxySQL HA 化（Cluster + runtime 同步 + 应用多端点）       | v1.10 | 待开始     |
| Redis Sentinel 边界修复（运行时拓扑 vs Ansible 静态配置）        | v1.11 | 待开始     |
| CI/CD 状态 IaC 化（Jenkins 镜像、Job、Credentials 备份边界） | v1.12 | 待开始     |
| 六节点基础接入（gz-04/gz-05 Tailscale + Docker + 监控纳管） | v1.13 | 待开始     |
| 服务迁移（gz-01/gz-02 角色重整 + watchdog）              | v1.14 | 待开始     |
| K3s 迁移无状态服务                                   | v1.15 | 待开始     |
| Helm Chart 管理 K3s 应用                          | v1.16 | 待开始     |
| Loki 日志集中管理                                   | v1.17 | 待开始     |
