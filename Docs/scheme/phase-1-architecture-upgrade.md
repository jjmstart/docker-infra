...
**路线调整**：v1.7 全量复盘中发现，当前 Ansible 已能稳定管理 Compose 配置和服务启动，但仍存在若干"历史一次性操作未纳入 IaC"的问题，例如 `mysql_source_delay` 已写入 inventory 但复制通道初始化仍依赖历史手工状态。为避免在扩节点 / K3s 迁移前继续放大这些隐性状态，v1.8-v1.11 先补齐架构可复现性与 HA 状态管理，原六节点接入及后续 K3s 路线顺延一格。

**v1.9 范围调整**：原 v1.9 "HA 配置状态修复"主题混合了 ProxySQL 配置 runtime 同步与 Redis Sentinel 边界修复两件落地动作完全不同的事，叙事不聚焦。结合 v1.7 IaC 审计中识别的 ProxySQL 进程级 SPOF（gz-03 整机宕机即应用读写全断），v1.9 重新定义为"**ProxySQL HA 化**"，把"双实例 Cluster + Admin SQL runtime 同步 + 应用 JDBC 多端点"合成一个完整的 ProxySQL HA 闭环；原 v1.9 中的 Redis Sentinel 边界修复（review #5 #6）独立成 v1.10。后续 CI/CD 状态 IaC 化等版本号顺延一格。
...
| ProxySQL HA 化（Cluster + runtime 同步 + 应用多端点）       | v1.9  | 待开始     |
| Redis Sentinel 边界修复（运行时拓扑 vs Ansible 静态配置）        | v1.10 | 待开始     |
| CI/CD 状态 IaC 化（Jenkins 镜像、Job、Credentials 备份边界） | v1.11 | 待开始     |
| 六节点基础接入（gz-04/gz-05 Tailscale + Docker + 监控纳管） | v1.12 | 待开始     |
| 服务迁移（gz-01/gz-02 角色重整）                        | v1.13 | 待开始     |
| K3s 迁移无状态服务                                   | v1.14 | 待开始     |
| Helm Chart 管理 K3s 应用                          | v1.15 | 待开始     |
| Loki 日志集中管理                                   | v1.16 | 待开始     |
...
这两件事本质同源：做 ProxySQL Cluster 双实例的前置就是打通 Admin SQL runtime 管理（两节点之间的配置同步走 Cluster 协议，必须经过 runtime DB），单独做 runtime 同步而不做 Cluster 没有完整闭环。v1.9 合并解决"进程级 HA"与"配置 runtime 同步"两层问题，把 ProxySQL HA 化做成一个完整的章节。
...
| gz-03 MySQL 进程挂、机器还在 | 写不可用 | 同 v1.9 前（**MySQL 自动 failover 不在本版本范围**） |
...
- v1.9 落地后在 `Docs/drills/` 新增 `v1.9-proxysql-failover.md`：kill gz-03 ProxySQL 容器，验证 JDBC 30s 内 failover 到 gz-02 ProxySQL；恢复后验证 Cluster 配置自动收敛
...
- 为什么"读 HA 路径"和"写 HA 路径"分开做（范围控制；v1.9 先把读路径做到自动 failover，写路径仍依赖人工 MySQL 切主，是有意识的灰度演进节奏）

---

## 任务 5：Redis Sentinel 边界修复（目标 v1.10）

### 背景

v1.9 把 ProxySQL 这层"运行时状态与 IaC 协同"的边界问题做透之后，Redis 侧还有一个完全对称的盲点（review #5 #6）：
...
v1.10 目标是修复这类"故障切换后 Ansible 重跑会破坏切换结果"的边界冲突。主题与 v1.9 同属"HA 系统运行时状态与 Ansible 静态配置的协同"，但落地动作完全不同（Redis 没有 Cluster 协议，是 conf 文件 + Sentinel runtime 自治），所以从 v1.9 拆出独立成版本。
...
- 结果记录到 `Docs/drills/v1.10-redis-sentinel-failover.md`
...
- runbook 清楚写明 Redis Sentinel 的运行时状态边界，与 v1.9 的 ProxySQL 边界共同构成"HA 状态边界"完整文档
...
## 任务 6：CI/CD 状态 IaC 化（目标 v1.11）

### 背景

bj-01 同时承担 Ansible 控制节点、Jenkins、Registry 和运维入口职责。当前 Jenkins 容器本身由 Ansible 管理，但 Jenkins 的关键内部状态仍依赖手工配置：自定义镜像 `jenkins-ansible:latest` 的构建过程、插件清单、Job、Credentials、`jenkins_home` 数据目录。v1.11 目标是把 CI/CD 平台自身也纳入"可备份、可恢复、可迁移"的管理边界。
...
## 任务 7：六节点基础接入（目标 v1.12）

### 背景

v1.11 已完成现有架构的基础可复现性、HA 状态边界和 CI/CD 状态管理修复，后续路线图（K3s 承载无状态服务、gz-02 角色重整）均需要额外节点支撑。v1.12 目标是将新购两台广州节点（gz-04、gz-05）安全、可重复地接入现有基础设施，**只做基础接入，不承载任何业务**：网络层纳入 Tailscale + Ansible IaC 管理，Docker 安装与基础运行时保持与现有节点一致，节点纳入 Prometheus 监控，新节点 SSH 采用 `admin-alex + become` 替代 root 直接登录还清技术债。
...
v1.12 完成 gz-04/gz-05 基础接入后，gz-04 具备接管 gz-02 副本职责的前提（v1.12 计划标注：gz-04"v1.13 接管 gz-02 副本职责"），gz-05 作为 K3s 控制面候选节点在 v1.13 保持空闲。v1.13 目标是在不下线现有服务的前提下，完成三步角色重整：① gz-04 接管 gz-02 的数据库/缓存副本职责；② gz-02 接管 gz-01 的 Nginx 入口与主监控职责；③ gz-01 降级为轻量 watchdog 节点，守住"监控主节点宕机时仍有人告警"这条可用性底线。

> **同步识别到的网关侧耦合（不在 v1.13 修复范围）**：当前 `roles/nginx/templates/docker-compose.yml.j2:16` 把若依前端静态目录以 `./ruoyi-ui-dist:/usr/share/nginx/ruoyi:ro` 形式直接挂载进 Nginx 容器，导致**网关 compose 文件持有业务知识**——新增前端业务必须改这个 compose 并重启 Nginx。v1.0 引入 Nginx 时只有一个前端，做静态托管是单业务场景下的合理简化；但 v1.13 起若有更多前端业务接入，这层耦合会越来越显性。v1.13 网关迁移会**原样**把这层耦合搬到 gz-02，不在本版本展开解耦；自然演进路径是 v1.14 K3s 落地后，前端走"独立 Nginx 镜像 + Ingress"，把"路由声明"从网关层下沉到 app 自身，那时再彻底解耦。中间状态如果出现新增前端业务，按现有方式补 conf.d 与 volume 即可，**不为此提前重构网关 role**。
...
- v1.13 阶段不部署任何业务；为 v1.14 K3s 控制面做准备，避免引入额外职责后给后续迁移增加复杂度
...
- gz-04 接管 gz-02、gz-05 留作 K3s 控制面——为何与 v1.12 草案预设的角色互换（根据节点后续职责调整：K3s 控制面需要保持专一，而承载 MySQL 从库的节点不适合同时成为 K3s master）
- 网关与业务前端的 volume 挂载耦合识别后**为何不在 v1.13 修复**（v1.13 网关迁移本身复杂度已足够，硬塞解耦会扩大失败面；解耦的最佳时机是 v1.14 K3s + Ingress 自然达成，体现"识别问题但克制处理"的工程取舍——既不忽视盲点，也不为"看起来专业"而越级重构）

---

## 任务 9：K3s 迁移无状态服务（目标 v1.14）

### 背景

当前若依后端（ruoyi-admin）以 Docker Compose 方式运行在 gz-03 和 gz-02 上。v1.14 目标是在不动有状态服务（MySQL、Redis、ProxySQL）的前提下，将若依后端迁移到 K3s，验证 Kubernetes 基础工作流。

### 要完成的内容

**K3s 部署**

- 选择 gz-05 作为 K3s 控制面节点（v1.12 已完成基础接入，运行时已就绪；v1.13 专为此保持空闲）
...
## 任务 10：Helm Chart 管理 K3s 应用（目标 v1.15）

### 背景

v1.14 的 K3s 应用以裸 YAML 方式管理。v1.15 目标是将若依后端的 Kubernetes YAML 封装成 Helm Chart，支持多环境参数化部署。
...
## 任务 11：Loki 日志集中管理（目标 v1.16）

### 背景

当前排障依赖各节点分散的 `docker logs`，没有集中日志检索能力。v1.16 目标是将 Docker Compose 和 K3s 的容器日志统一采集到 Loki，通过 Grafana 实现跨节点日志查询。
...
## 任务 12：可观测性增强（目标 v2.0）

### 背景

到 v1.16 为止，监控（Prometheus）、告警（Alertmanager）、日志（Loki）已各自成型但相互独立。v2.0 目标是在 Grafana 中将三者整合，形成"指标异常 → 定位时间点 → 关联日志 → 追查根因"的完整排障链路。
...
- 横向任务**不依赖** v1.X 子任务进度，可独立推进，但下游会引用：例如 v1.9 ProxySQL HA 化落地后，应在 `Docs/drills/` 新增一份 ProxySQL 单实例故障切换演练，验证 JDBC 多端点 failover；v1.10 Redis Sentinel 边界修复落地后，类似补一份 Sentinel 选主后 Ansible 重跑验证演练
