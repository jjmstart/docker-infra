...
> **同步识别到的网关侧耦合（不在 v1.12 修复范围）**：当前 `roles/nginx/templates/docker-compose.yml.j2:16` 把若依前端静态目录以 `./ruoyi-ui-dist:/usr/share/nginx/ruoyi:ro` 形式直接挂载进 Nginx 容器，导致**网关 compose 文件持有业务知识**——新增前端业务必须改这个 compose 并重启 Nginx。v1.0 引入 Nginx 时只有一个前端，做静态托管是单业务场景下的合理简化；但 v1.12 起若有更多前端业务接入，这层耦合会越来越显性。v1.12 网关迁移会**原样**把这层耦合搬到 gz-02，不在本版本展开解耦；自然演进路径是 v1.13 K3s 落地后，前端走"独立 Nginx 镜像 + Ingress"，把"路由声明"从网关层下沉到 app 自身，那时再彻底解耦。中间状态如果出现新增前端业务，按现有方式补 conf.d 与 volume 即可，**不为此提前重构网关 role**。
...
- 网关与业务前端的 volume 挂载耦合识别后**为何不在 v1.12 修复**（v1.12 网关迁移本身复杂度已足够，硬塞解耦会扩大失败面；解耦的最佳时机是 v1.13 K3s + Ingress 自然达成，体现"识别问题但克制处理"的工程取舍——既不忽视盲点，也不为"看起来专业"而越级重构）
