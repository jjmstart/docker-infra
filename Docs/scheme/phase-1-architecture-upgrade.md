...
[`service-migration`](#service-migration) 主题中的 watchdog 设计只解决"采集 / 评估端"的冗余（监控主节点宕机时仍有备份采集），watchdog 自己也是把告警发给同一个 Alertmanager，**完全没覆盖"告警出口端"单点**——即"who watches the watcher"问题在自己的设计里再嵌套了一层"who watches the watcher's mouth"。完整的告警链路按独立失效域应拆 4 段：采集 / 评估 / 出口 / 送达，watchdog 只覆盖第 1 段，2-4 段全单点。
...
- 为什么"读 HA 路径"和"写 HA 路径"分开做（范围控制；本主题先把读路径做到自动 failover，写路径仍依赖人工 MySQL 切主，是有意识的灰度演进节奏）
...
bj-01 同时承担 Ansible 控制节点、Jenkins、Registry 和运维入口职责。当前 Jenkins 容器本身由 Ansible 管理，但 Jenkins 的关键内部状态仍依赖手工配置：自定义镜像 `jenkins-ansible:latest` 的构建过程、插件清单、Job、Credentials、`jenkins_home` 数据目录。本主题目标是把 CI/CD 平台自身也纳入"可备份、可恢复、可迁移"的管理边界。
...
[`cicd-state-iac`](#cicd-state-iac) 主题已完成现有架构的基础可复现性、HA 状态边界和 CI/CD 状态管理修复，后续路线图（K3s 承载无状态服务、gz-02 角色重整）均需要额外节点支撑。本主题目标是将新购两台广州节点（gz-04、gz-05）安全、可重复地接入现有基础设施，**只做基础接入，不承载任何业务**：网络层纳入 Tailscale + Ansible IaC 管理，Docker 安装与基础运行时保持与现有节点一致，节点纳入 Prometheus 监控，新节点 SSH 采用 `admin-alex + become` 替代 root 直接登录还清技术债。
...
- `roles/docker-daemon/`：在现有 pip/SDK/daemon.json 配置任务之前**插入 Docker Engine 安装任务**（docker-ce + docker-compose-plugin），保持幂等；`global_gateway` 网络创建沿用 [`base-reproducibility-fix`](#base-reproducibility-fix) 主题已纳入的管理逻辑，gz-04/gz-05 走相同 role 时自动具备该网络；现有四台节点重复执行 `changed=0`
...
[`six-node-onboarding`](#six-node-onboarding) 主题完成 gz-04/gz-05 基础接入后，gz-04 具备接管 gz-02 副本职责的前提（该主题计划标注：gz-04 在本主题接管 gz-02 副本职责），gz-05 作为 [`k3s-stateless`](#k3s-stateless) 主题的控制面候选节点在本主题保持空闲。本主题目标是在不下线现有服务的前提下，完成三步角色重整：① gz-04 接管 gz-02 的数据库/缓存副本职责；② gz-02 接管 gz-01 的 Nginx 入口与主监控职责；③ gz-01 降级为轻量 watchdog 节点，守住"监控主节点宕机时仍有人告警"这条可用性底线。

> **同步识别到的网关侧耦合（不在本主题修复范围）**：当前 `roles/nginx/templates/docker-compose.yml.j2:16` 把若依前端静态目录以 `./ruoyi-ui-dist:/usr/share/nginx/ruoyi:ro` 形式直接挂载进 Nginx 容器，导致**网关 compose 文件持有业务知识**——新增前端业务必须改这个 compose 并重启 Nginx。v1.0 引入 Nginx 时只有一个前端，做静态托管是单业务场景下的合理简化；但本主题落地起若有更多前端业务接入，这层耦合会越来越显性。本主题网关迁移会**原样**把这层耦合搬到 gz-02，不在本主题展开解耦；自然演进路径是 [`k3s-stateless`](#k3s-stateless) 主题落地后，前端走"独立 Nginx 镜像 + Ingress"，把"路由声明"从网关层下沉到 app 自身，那时再彻底解耦。中间状态如果出现新增前端业务，按现有方式补 conf.d 与 volume 即可，**不为此提前重构网关 role**。
...
- **覆盖边界声明**：watchdog 仅覆盖**采集端 / 评估端**单点（gz-02 主监控节点宕机时仍有备份采集），不覆盖**告警出口端**单点（Alertmanager / prometheus-alert / bj-01 整机）。出口端冗余由 [`dms-outlet-redundancy`](#dms-outlet-redundancy) 主题引入的 Dead Man's Switch 通过外部 SaaS 反向心跳兜底。两者是互补关系，不可替代。详见 `Docs/reviews/v1.7-alerting-pipeline-spof-audit.md`。

**gz-05 保持空闲**

- 本主题阶段不部署任何业务；为 [`k3s-stateless`](#k3s-stateless) 主题 K3s 控制面做准备，避免引入额外职责后给后续迁移增加复杂度
...
- 为什么 watchdog 仍需要 [`dms-outlet-redundancy`](#dms-outlet-redundancy) 主题 DMS 兜底（watchdog 只覆盖采集端单点，告警出口端单点必须由 DMS 通过外部 SaaS 反向心跳覆盖；HA 设计要按独立失效域分别审视而非按组件名整体处理）
- 服务迁移为什么走"旁路部署 → DNS 切换 → 降级旧节点"三步而非直接替换（保证零中断、任一步失败可回滚，体现灰度发布思路）
- gz-04 接管 gz-02、gz-05 留作 K3s 控制面——为何与 [`six-node-onboarding`](#six-node-onboarding) 主题草案预设的角色互换（根据节点后续职责调整：K3s 控制面需要保持专一，而承载 MySQL 从库的节点不适合同时成为 K3s master）
- 网关与业务前端的 volume 挂载耦合识别后**为何不在本主题修复**（本主题网关迁移本身复杂度已足够，硬塞解耦会扩大失败面；解耦的最佳时机是 [`k3s-stateless`](#k3s-stateless) 主题 K3s + Ingress 自然达成，体现"识别问题但克制处理"的工程取舍——既不忽视盲点，也不为"看起来专业"而越级重构）
...
当前若依后端（ruoyi-admin）以 Docker Compose 方式运行在 gz-03 和 gz-02 上。本主题目标是在不动有状态服务（MySQL、Redis、ProxySQL）的前提下，将若依后端迁移到 K3s，验证 Kubernetes 基础工作流。

### 要完成的内容

**K3s 部署**

- 选择 gz-05 作为 K3s 控制面节点（[`six-node-onboarding`](#six-node-onboarding) 主题已完成基础接入，运行时已就绪；[`service-migration`](#service-migration) 主题专为此保持空闲）
...
[`k3s-stateless`](#k3s-stateless) 主题的 K3s 应用以裸 YAML 方式管理。本主题目标是将若依后端的 Kubernetes YAML 封装成 Helm Chart，支持多环境参数化部署。
...
当前排障依赖各节点分散的 `docker logs`，没有集中日志检索能力。本主题目标是将 Docker Compose 和 K3s 的容器日志统一采集到 Loki，通过 Grafana 实现跨节点日志查询。
...
到 [`loki-logging`](#loki-logging) 主题为止，监控（Prometheus）、告警（Alertmanager）、日志（Loki）已各自成型但相互独立。本主题目标是在 Grafana 中将三者整合，形成"指标异常 → 定位时间点 → 关联日志 → 追查根因"的完整排障链路。
...
- 不做全节点统一代理
- 不做业务流量透明代理
- 不把代理作为当前主线任务的硬性前置条件（当前主线起点为 [`dms-outlet-redundancy`](#dms-outlet-redundancy) 主题）
