...
- 更新 `vault/secrets.yml`，新增飞书 webhook URL 与签名 secret，真实值必须使用 ansible-vault 加密保存。
...
飞书 webhook URL 和签名 secret 必须进入 `vault/secrets.yml`，由 ansible-vault 加密管理。草案、runbook、architecture 和 Git 普通变量文件不得记录真实 token 或 secret。

建议变量名：

```yaml
alerting_feishu_webhook_url: "<由 ansible-vault 管理>"
alerting_feishu_secret: "<由 ansible-vault 管理>"
```
...
- 飞书 webhook URL 和签名 secret 未出现在普通配置文件和文档中。
