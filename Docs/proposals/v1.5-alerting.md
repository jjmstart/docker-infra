...
```yaml
alerting_feishu_webhook_token: "<由 ansible-vault 管理>"
```

当前建议选用的 `bougou/alertmanager-webhook-adapter` 的 `--signature` 是消息前缀，不是飞书机器人签名校验。V1.5 建议在飞书机器人安全设置中使用“自定义关键词”，关键词包含 `docker-infra`；如后续必须启用飞书签名校验，可在 V1.6 更换 adapter 或自维护极简 adapter。
