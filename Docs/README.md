...
| [proposals/v1.10-proxysql-ha.md](proposals/v1.10-proxysql-ha.md) | V1.10 | ProxySQL HA 化设计草案（占位，仅 PoC 待确认问题已填充） |
...
| [reviews/v1.7-alerting-pipeline-spof-audit.md](reviews/v1.7-alerting-pipeline-spof-audit.md) | V1.7 | 告警链路 SPOF 盲点：watchdog 仅覆盖采集端、未覆盖告警出口；按 4 个独立失效域审视后，DMS 独立成 v1.8 覆盖 #2/#3/#4，watchdog 保留在顺延后的 v1.14 仅覆盖 #1 |
...
| [narratives/v1.7.md](narratives/v1.7.md)     | V1.7 | 10     | IaC 完整性 / CI 质量门禁 / 网关耦合演进时机 / 入口探测功能深度盲区 / Jenkins 403 双层语义 / Prometheus reload inode 陷阱 / Jenkins CD 路径过滤层级试错 / role 镜像 tag 收敛 / ProxySQL SPOF 路线重划 / 告警链路 watchdog 二阶盲点 + DMS 互补（含一句话版、Q&A、工程原则） |
