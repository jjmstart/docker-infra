...
**与 SRE 经典 SLO 的边界说明**：本项目当前为单人维护的学习集群，不存在多团队责任划分、不需要错误预算燃烧率（burn-rate）告警、不签订 SLA 对外赔付。本文件保留 SLI/SLO 框架但**不引入完整 SRE 错误预算策略**，避免在当前规模下堆砌无法对应运营动作的指标——保留"指标定义 + 月度目标 + 违约时怎么办"三段最小核心。
...
**单副本窗口与可用性预算的边界**（v1.9 drain 演练补测校准）：计划内单 worker 维护（如 `kubectl drain`）会让若依 K3s 副本短时降到单副本，但只要存活副本健康、`probe_success` 仍为 1，**该窗口不计入若依不可用时间**——它消耗的是"冗余度"而非"可用性"。2026-06-12 drain 演练实测：单副本窗口 ≈50s 内公网探针 305 次全 `200`（见 `Docs/drills/v1.9-k3s-failure-drills.md` §4.4）。当前 SLO 仅以 `probe_success` 衡量"可不可达"，**不单独度量副本冗余度**——这是有意识的范围取舍（单人学习集群不为数十秒窗口堆砌冗余度 SLI），代价是"维护期零冗余"这一脆弱状态在 SLO 指标上不可见，需在执行维护操作时人工知悉"此刻再挂一个副本即中断"。
...
| 演练未达标 | 季度未完成至少 1 次演练 | 优先级提升至当前架构演进副线之上，强制补做一次 |
