...
| [v1.8-to-v1.9.md](v1.8-to-v1.9.md) | K3s 迁移无状态服务：gz-06 onboarding（Phase 0）+ gz-05 server + gz-04/gz-06 agent 三节点 K3s；ruoyi 裸 YAML（Deployment 2 副本跨 worker + NodePort + ConfigMap/Secret）；gz-01 Nginx upstream 变量驱动切 NodePort；三类故障演练；Compose 停而不删 3 天窗口（12 个 Phase / 集群·迁移·摘除三次提交检查点） | gz-06（新接入常驻 worker：腾讯云 Ubuntu 24.04 4C4G）、gz-05（K3s server）、gz-04/gz-06（agent + ruoyi Pod）、gz-01（Nginx 切流）、gz-02/gz-03（Compose 老副本摘除）、bj-01（kubectl/apply 入口） |
