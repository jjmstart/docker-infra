...
| [v1.7-to-v1.8.md](v1.7-to-v1.8.md) | 六节点基础接入：gz-04/gz-05 加入 Tailscale + 装 Docker Engine + 纳入 node-exporter；新增 base-access role 在 6 台纯追加 admin-alex；docker-daemon 扩展管理 Docker 安装与 global_gateway 网络（8 个 Phase / 框架·接入·监控·验收四个提交检查点） | gz-04/gz-05（新接入：百度云 Ubuntu 24.04 / 腾讯云 Ubuntu 22.04）、gz-01/gz-02/gz-03/bj-01（base-access 纯追加 admin-alex）、gz-01（Prometheus 重渲染） |
| [v1.8-to-v1.9.md](v1.8-to-v1.9.md) | K3s 迁移无状态服务：gz-06 onboarding（Phase 0）+ gz-05 server + gz-04/gz-06 agent 三节点 K3s；ruoyi 裸 YAML（Deployment 2 副本跨 worker + NodePort + ConfigMap/Secret）；gz-01 Nginx upstream 变量驱动切 NodePort；四类故障演练（含 reboot 冷启动自愈）；Compose 停而不删 → 条件放行（替代 3 天日历窗口）+ 有界冷备 7 天（12 个 Phase / 集群·迁移·摘除三次提交检查点） | gz-06（新接入常驻 worker：腾讯云 Ubuntu 24.04 4C4G）、gz-05（K3s server）、gz-04/gz-06（agent + ruoyi Pod）、gz-01（Nginx 切流）、gz-02/gz-03（Compose 老副本停而不删，有界冷备）、bj-01（kubectl/apply 入口） |
