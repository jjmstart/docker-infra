...
| 服务 | 容器名 | 端口 | 说明 |
|------|--------|------|------|
| Nginx | nginx | 80, 443 | 备案通过后 HTTPS 已切换为标准 443 端口 |
...
Mermaid 架构图（拆分为三张，避免单图线条过密导致文字重叠）：
...
flowchart LR
    ...
    subgraph BJ01["bj-01 · 运维 / 灾备 / Ansible 控制节点<br/>100.118.69.78 · 跨城约 35ms"]
        direction TB
...
```mermaid
flowchart LR
    subgraph DB["MySQL 主从复制"]
        direction LR
        MYM["gz-03<br/>MySQL Master<br/>server-id=1"]
        MYS1["gz-02<br/>MySQL Slave1<br/>实时只读"]
        MYS2["bj-01<br/>MySQL Slave2<br/>延迟 3600s"]
        MYM -->|"半同步 · 同城 ~5ms"| MYS1
        MYM -->|"半同步 · 跨城 ~35ms"| MYS2
    end

    subgraph REDIS["Redis 复制 + Sentinel"]
        direction LR
        RM["gz-03<br/>Redis Master"]
        RS1["gz-02<br/>Redis Slave1"]
        RS2["bj-01<br/>Redis Slave2"]
        S1["Sentinel1"]
        S2["Sentinel2"]
        S3["Sentinel3"]
...
flowchart LR
    ...
    subgraph TARGETS["被管理节点"]
        direction TB
        GZ01_T["gz-01<br/>/opt/docker"]
        GZ02_T["gz-02<br/>/opt/docker"]
        GZ03_T["gz-03<br/>/opt/docker"]
        BJ01_T["bj-01 本机服务<br/>/opt/docker"]
    end
    ...
    REPO --> AP
    VAULT --> AP
    AP -->|"SSH via Tailscale<br/>配置渲染 + 精准下发 + handler 重启"| GZ01_T
    AP -->|"SSH via Tailscale<br/>配置渲染 + 精准下发 + handler 重启"| GZ02_T
    AP -->|"SSH via Tailscale<br/>配置渲染 + 精准下发 + handler 重启"| GZ03_T
    AP -->|"本机执行<br/>同样由 Ansible 纳管"| BJ01_T
```
